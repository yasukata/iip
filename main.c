										...
										while (1) {
											...
											if (conn->seq_next_expected != SEQ_LE_RAW(_p)) { /* sequence number is different from expected one, but keep in head[4] */
																	...
																	if (SEQ_LE(_p) == SEQ_LE(__p)) {
																		...
																		if (SEQ_RE(_p) <= SEQ_RE(__p)) {
																			...
																			if (_p->a_cnt - __p->a_cnt < 2147483648U) {
																				...
																				__p->tcp.inc_head += PB_TCP_HDR_HAS_SYN(_p->pkt) + PB_TCP_HDR_HAS_FIN(_p->pkt) + PB_TCP_PAYLOAD_LEN(_p->pkt) - _p->tcp.dec_tail;
												...
												if (/* PAWS */ ((p->flags & __IIP_PB_FLAGS_OPT_HAS_TS) && !PB_TCP_HDR_HAS_SYN(p->pkt) && !PB_TCP_HDR_HAS_RST(p->pkt)) && (2147483648U <= (p->tcp.opt.ts[0] < conn->ts ? conn->ts - p->tcp.opt.ts[0] : p->tcp.opt.ts[0] - conn->ts))) {
													...
													__iip_assert(conn->seq_next_expected == __iip_ntohl(PB_TCP(_p->pkt)->seq_be) + PB_TCP_HDR_HAS_SYN(_p->pkt) + PB_TCP_HDR_HAS_FIN(_p->pkt) + PB_TCP_PAYLOAD_LEN(_p->pkt) - _p->tcp.dec_tail - _p->tcp.inc_head);
													if (conn->head[4][0]) {
														__iip_assert(conn->head[4][1]);
														if (p == _p) {
